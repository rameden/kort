...
module.exports = function(passport, flash, googleClientID, googleClientSecret, googleCallbackURL) {
    ...
    function(token, refreshToken, profile, done) {
        ...
        process.nextTick(function() {
            ...
            // Google accounts can have multiple email addresses thus the "in" search
            User.findOne({ 'email' : { $in: profile.emails } }, function(err, user) {
                if (err) {
                ...
                if (user) {
					if (user.type == 'local') {
						// an existing local user is found, boot them back
						return done(null, false, req.flash('createUserErrorMessage', 'Email is already taken, try using the main login screen.'));
					} else {
                    ...
                    newUser.save(function(err) {
                        ...
                        return done(null, newUser, req.flash('createUserSuccessMessage', 'Account created successfully.'));
                    });
                }
            });
        });
